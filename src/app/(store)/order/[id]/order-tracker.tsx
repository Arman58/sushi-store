...
import { ModifiersList, parseSelectedModifiersJson } from "@/features/cart";
...
export function OrderTracker({ order: initial, phone }: OrderTrackerProps) {
    const { data } = useQuery<OrderStatusResponse>({
        ...
        refetchInterval: (query) => {
            ...
            return data?.status === "NEW" || data?.status === "IN_WORK" || data?.status === "DELIVERING"
                ? 10000
                : false;
        },
    });
        ...
        >
            ...
            <Paper sx={{ mt: 4, p: 2, borderRadius: 3 }} variant="outlined">
                ...
                {order.items.map((item) => {
                    ...
                    return (
                        <Stack key={item.id} spacing={0.25} sx={{ mt: 1 }}>
                            <Stack direction="row" justifyContent="space-between">
                                ...
                                <Typography
                                    fontWeight={600}
                                    sx={{ fontSize: 17, whiteSpace: "nowrap", pl: 2 }}
                                >
                                    {(item.price * item.quantity).toLocaleString("ru-RU")}{" "}
                                    ֏
                                </Typography>
                            </Stack>
                            <ModifiersList modifiers={mods} sx={{ pl: 0.5 }} />
                        </Stack>
                    );
                })}
